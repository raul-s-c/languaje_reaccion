...
class WhisperContext private constructor(private var ptr: Long) {
    ...
    suspend fun transcribeSegments(data: FloatArray): List<WhisperSegment> = withContext(scope.coroutineContext) {
        require(ptr != 0L)
        ...
        return@withContext buildList {
            for (index in 0 until textCount) {
                add(
                    WhisperSegment(
                        startMillis = WhisperLib.getTextSegmentT0(ptr, index) * 10L,
                        endMillis = WhisperLib.getTextSegmentT1(ptr, index) * 10L,
    ...
    suspend fun transcribeData(data: FloatArray, printTimestamp: Boolean = true): String {
        val segments = transcribeSegments(data)
        return buildString {
            segments.forEach { segment ->
                if (printTimestamp) {
                    append("[${toTimestamp(segment.startMillis / 10)} --> ${toTimestamp(segment.endMillis / 10)}]: ")
                }
